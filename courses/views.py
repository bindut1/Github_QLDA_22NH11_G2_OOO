...
def student_list_by_course(request, course_id):
    ...
    for schedule in schedules:
        ...
        attendance = Attendance.objects.filter(
            student=student, course=course, check_in_date=current_date
        ).first()
        is_attended = attendance is not None
        if is_attended:
            attended_count += 1
        students_data.append(
            {"student": student, "is_attended": is_attended, "attendance": attendance}
        )
...
def export_attendance(request, course_id):
    ...
    header_style = xlwt.easyxf(
        "font: bold on; align: wrap on, vert centre, horiz center; pattern: pattern solid, fore_color light_blue;"
    )
    ...
    attended_count = Attendance.objects.filter(
        course=course, check_in_date=current_date
    ).count()
...
def student_schedule_week(request, account_id):
    ...
    for date in week_dates:
        ...
        weekly_schedule.append(
            {"date": date, "day_name": day_name, "courses": daily_courses}
        )
